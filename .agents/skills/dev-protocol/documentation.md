...
| **`.agents/lessons-learned.md`** | New technical invariant of *this* product (render, UI, provider, perf). File lives **next to** `skills/`, not inside the skill. | Process-only changes. If the file does not exist, skip — do not create it inside `dev-protocol/`. |
