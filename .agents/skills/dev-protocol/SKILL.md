...
> **Portability Note**: Esta skill es un **template de proceso**, agnóstico al producto. No contiene rutas absolutas, ni invariantes de un repo, máquina o usuario específico. Para adoptarla en otra app con el mismo stack (Python/`uv` + LLM), copiá **solo** el directorio `dev-protocol/` a `.agents/skills/` del nuevo repo y symlinkealo a `.claude/skills/`. La memoria del producto (`lessons-learned.md`) **no** viaja con la skill — vive en el repo host. Todas las referencias internas de la skill son relativas.
...
### Memoria del producto (fuera de esta skill)

No es un módulo. **No escribas invariantes de producto dentro de `dev-protocol/`.**

Buscá `.agents/lessons-learned.md` (hermano de `skills/`). Si existe, leelo al arrancar una implementación y registrá ahí las invariantes nuevas. Si no existe, **skip** — no lo inventes ni lo creés adentro de esta skill.
...
1. **Cargar y orientar**: leé este SKILL.md primero, después los módulos relevantes. Si el repo tiene `.agents/lessons-learned.md`, leelo; si no, seguí.
