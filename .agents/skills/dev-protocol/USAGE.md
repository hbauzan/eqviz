...
```
.agents/
├─ lessons-learned.md   ← MEMORIA DEL REPO (producto). No se copia con la skill.
│                         Si no existe, skip.
└─ skills/
   └─ dev-protocol/     ← TEMPLATE DE PROCESO. Esto sí se copia a otra app.
...
- **Lecciones ≠ skill.** Copiar `dev-protocol/` a otro repo **no** debe llevar `.agents/lessons-learned.md`. Ese archivo es memoria del producto host.
...
- Si existe `.agents/lessons-learned.md` (fuera de la skill), consultalo; si no, skip.
