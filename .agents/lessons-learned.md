...
- **Palettes (09 + 90s Sony, 2026-08-27):** six `VisualizerStyle` cases. **90s Sony**: muted icy LED `sony90sPhosphor` `(0.52, 0.70, 0.82)`. Bar **body** uses shared `PeakDecay` (gravity `1.2`, hold `0`) like the other styles. Separate `tipPeaks` / `tipDecay` (hold `0.5` s, gravity `0.5`) drives one half-square tip. Smoked backdrop; fake glow. CPU roadmap 15–17 paused.
