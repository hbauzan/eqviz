...
- **Palettes (09 + 90s Sony, 2026-08-27):** six `VisualizerStyle` cases. Labels: `"Retro Red"` … `"Cyber Neon"`, `"90s Sony"`. Default remains `.retroRed`. **90s Sony**: muted phosphor `sony90sPhosphor` `(0.12, 0.55, 0.48)` for body **and** tip (no overload red). Top of each bar is **one** half-height cell (`tipHalfRect`), not two red segments. Smoked opaque backdrop; fake glow without blur. Peak profile style-only: hold `0.5` s + gravity `0.5`. `AudioEngine.tickPeaks(at:style:)`. CPU roadmap 15–17 paused for this feature.
