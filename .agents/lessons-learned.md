...
- **Peak decay (07 + 90s Sony hold, 2026-08-27):** `PeakDecay.tick(bands:dt:)` — instant attack; optional per-band hold then linear gravity. Default: hold `0`, gravity `1.2`. **90s Sony:** hold `0.5` s, gravity `0.5`. If `holdDuration == 0`, fall immediately (legacy). Display clock only. `AudioEngine.peaks` lock snapshot; not `@Observable`.
- **Peak decay (07, 2026-08-20):** instant attack, linear gravity `1.2` /s. Display clock only (`TimelineView` wrapping **only** the Canvas). **Forbidden:** `withAnimation`, `Timer.scheduledTimer(1/120)`, decaying in the audio callback.
...
- **Palettes (09 + 90s Sony, 2026-08-27):** six `VisualizerStyle` cases. Labels: `"Retro Red"` … `"Cyber Neon"`, `"90s Sony"`. Default remains `.retroRed`. **90s Sony** (least-effort VFD): body turquoise `(0.05, 0.95, 0.82)`, overload segments `14…15` amber-red `(1.0, 0.32, 0.04)`, smoked opaque backdrop `(0.04, 0.055, 0.05)` (not window transparency), fake glow = expanded dim rect behind lit cells (no blur/`drawingGroup`). Peak profile style-only: hold `0.5` s + gravity `0.5`; other styles keep hold `0` + gravity `1.2`. `AudioEngine.tickPeaks(at:style:)` configures `PeakDecay` each display tick. CPU roadmap 15–17 paused for this feature.
- **Palettes (09, 2026-08-20):** one `VisualizerView`, five base styles before 90s Sony. Colors live in `VisualizerPalette.rgb` — copy locked hues; do not invent. Changing style must not call `engine.stop()`. No UserDefaults. Unlit RGB still exists for tests; **paint skips unlit** (14). Fire and cyber lerp **by segment**; rainbow hue **by band**.
